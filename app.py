...
def index():
    if "game_id" not in session:
        ...
        session["game_state"] = pickle.dumps(game)   # ゲーム状態をセッションに保存
    else:
        # 既存のゲーム状態をセッションから取得
        game = pickle.loads(session.get("game_state", pickle.dumps(ZoneGame())))  # デシリアライズ  # セッションに状態がなければ新しいゲーム
    return render_template("index.html")

@app.route("/move", methods=["POST"])
def move():
    game = pickle.loads(session.get("game_state", pickle.dumps(ZoneGame())))
    data = request.json
    x, y = data["x"], data["y"]
